 ...
 *
 * Original code is based on this modified
 * [posemsg_to_file](https://github.com/rpng/posemsg_to_file/). Output is in a
 * text file that is space deliminated and can be read by all scripts. If we
 * have a covariance then we also save the upper triangular part to file so we
 * can calculate NEES values.
 */
class Recorder {
...
public:
   ...
   * If the output directory does not exists this will also create the directory
   * path to this file.
   ...
   *
   * Note that covariance is in the order (x, y, z, rotation about X axis,
  ...
  void callback_odometry(const nav_msgs::OdometryPtr &msg) {
    ...
    cov_pos << msg->pose.covariance.at(0), msg->pose.covariance.at(1),
        msg->pose.covariance.at(2), msg->pose.covariance.at(6),
        msg->pose.covariance.at(7), msg->pose.covariance.at(8),
        msg->pose.covariance.at(12), msg->pose.covariance.at(13),
        msg->pose.covariance.at(14);
    cov_rot << msg->pose.covariance.at(21), msg->pose.covariance.at(22),
        msg->pose.covariance.at(23), msg->pose.covariance.at(27),
        msg->pose.covariance.at(28), msg->pose.covariance.at(29),
        msg->pose.covariance.at(33), msg->pose.covariance.at(34),
...
protected:
   ...
   * This should be called after we have saved the desired pose to our class
   * variables.
   */
  void write() {
    ...
    if (has_covariance) {
      ...
      outfile << " " << cov_rot(0, 0) << " " << cov_rot(0, 1) << " "
              << cov_rot(0, 2) << " " << cov_rot(1, 1) << " " << cov_rot(1, 2)
              << " " << cov_rot(2, 2) << " " << cov_pos(0, 0) << " "
              << cov_pos(0, 1) << " " << cov_pos(0, 2) << " " << cov_pos(1, 1)
